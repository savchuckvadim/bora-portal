                ...
                if (ts - lastDetect >= 33) {
                    ...
                    try {
                        ...
                        if (res && res.detection) {
                            ...
                            const look = new THREE.Vector2(Math.tanh(nxm ?? 0), Math.tanh(nym ?? 0));
                            ...
                            const tilt = Math.atan2((rc?.y ?? 0) - (lc?.y ?? 0), (rc?.x ?? 0) - (lc?.x ?? 0));
                            medianPush(bufT.current, tilt, 5);
                            tiltTarget.current = median(bufT.current) ?? 0;
                            ...
                            if (tip && leftCheek && rightCheek && lc && rc) {
                                ...
                                yawTarget.current = median(bufYaw.current) ?? 0;
                            }
...
function median(buf: number[]) {
    ...
    const a = [...buf].sort((x: number, y: number) => x - y);
    const mid = Math.floor(a.length / 2);
    return a.length % 2 ? (a[mid] ?? 0) : ((a[mid - 1] ?? 0) + (a[mid] ?? 0)) * 0.5;
}
